...
resource "azurerm_key_vault_secret" "database_username_group1" {
  name         = "db-username-ws-group1"
  value        = random_string.mssql_user_database_username_group1.result
  key_vault_id = var.key_vault.id
}

resource "azurerm_key_vault_secret" "database_password_group1" {
  name         = "db-password-ws-group1"
...
resource "mssql_login" "dblogin" {
  ...
  login_name       = azurerm_key_vault_secret.database_username_group1.value
  password         = azurerm_key_vault_secret.database_password_group1.value
...
resource "mssql_user" "dbuser" {
  ...
  username   = "user-${azurerm_key_vault_secret.database_username_group1.value}"
  login_name = azurerm_key_vault_secret.database_username_group1.value
  database   = "ws-group1"
  roles      = ["db_owner"]
}
